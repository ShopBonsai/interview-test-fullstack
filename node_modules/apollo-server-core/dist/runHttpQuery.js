...
function throwHttpGraphQLError(statusCode, errors, options) {
    ...
    const headers = apollo_server_errors_1.hasPersistedQueryError(errors)
        ? Object.assign({}, defaultHeaders, { 'Cache-Control': 'private, no-cache, must-revalidate' }) : defaultHeaders;
            ...
            if (Array.isArray(requestPayload)) {
                ...
                try {
                    ...
                    if (response.errors && typeof response.data === 'undefined') {
                        return throwHttpGraphQLError((response.http && response.http.status) || 400, response.errors);
                    }
                    if (response.http) {
